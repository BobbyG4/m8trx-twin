...
>
> **✅ MERGED 2026-07-30 — PR #13, `main` now `39c8238`** (branch `chore/spine-restart-hygiene`, 15 commits; was `b0390fc`). No open PRs, tree clean. All green: ktlint · compile · `connectSelfTest` · `peopleSelfTest` (49) · `scenarioSelfTest` (23).
>
> **S16 (2026-07-30) — coordinator lane brief picked up** (`m8trx-shared/status/briefs/BRIEF-TWIN-LANE-2026-07-30.md`, p2, nothing blocking). Answered §1 **loudly: the `person_session` orphan close was NOT twin** — no twin code path closes a session (`RestEmitter.kt:36` is the only `personSessionClose` and has **zero callers**), psql is Off-Limits, twin ran nothing after `2026-07-28 15:57`, and the core-side backup CSV covers **office** orphans, not Denver → **someone else touched the Denver rows and there is no backup for them.** Full evidence chain in `status/tracks/TRACK-TWIN.md` §Current State. Ledger reconciled: **TWIN-REQ-004 → ABSORBED core-side**; **CORE-REQ-001 was already ABSORBED** twin-side since 2026-06-21 (the coordinator's flag is stale on their copy).
>
...
>
> **NEXT, ranked:**
> **(1) ✅ DONE 2026-07-30 — fixture coverage 97/115 → 115/115.** The 18 never-browsed fixtures (incl. all three circles) are fixed, and the dig turned up **two independent root causes, not one**: (a) `dwellAtZone` reached fixtures only via `fixturesByDept`, and 19 of Denver's 115 carry no department — now keyed off **`in_area_zone`** (verified non-null + orphan-free on every fixture in all 10 stores), paired with a new **`fixtureBrowseRate(role)`** so zone-keying doesn't swing the other way and park all 790 shoppers at the entrance circles; (b) **`ACC-02` and `GPS-04` were unreachable by construction** — boxed in on all four sides with 200–400mm gaps against a fixed 600mm standoff, so every candidate landed *inside* a neighbour, nothing ray-validated, and the fallback aimed the shopper at the neighbour (the paired-gondola bug from the opposite direction). Standoff is now fitted to each edge's clearance (`min(600, clearance/2)`, edges under 150mm dropped, jitter scaled to match) — **identical to the proven behaviour wherever there is room.** Regression is deterministic and scale-free: `peopleSelfTest` **[10] all 115 individually browsable** (49 → **50**); `scenarioSelfTest` adds coverage + circles + department-less cases (23 → **27**). Full-scale day: **115/115 covered · 792 visitors · 1,177,671 samples (+7.0%) · 3,967 predicted impressions (+8.3%)**. ⚠ **Not fixed, flagged:** those 200–400mm gaps between 2–3m display blocks are not walkable aisles — a **layout-realism** defect in the twin dataset, deliberately left alone because regenerating layouts would break the byte-for-byte reseed parity with mother.
> **(2) ✅ DONE 2026-07-30 — DB verification delivered, and it found a REAL gap.** 3,119 Denver rows: `fullday-0728` **2,256 vs 3,664 over NATS = 1,408 lost (38.4%)** · `evening-peak` **854 vs 854 = 0%** · tests 9. Twin answered the split (**the 3,664 was fullday-only, so the loss is 1,408, not 545**) and built `lossAudit`, which **falsified two hypotheses** before they could cost core time: concurrency/cache-capacity (peak is ~169 entries and is duration-*independent* under arrival compression) and dedup-on-`(session,fixture)` (repeats are only 5.6%). **Hard fact handed over:** `person_session` persisted **790/790** in the same run that lost 38.4% of impressions → the gap is on the **impression path, downstream of ingest, not the rule**. Core's investigation, filed with the BW lane. ⚠ **Wording correction:** "the oracle was EXACT" was only ever exact against the **emitted stream** — it was never a persistence claim, and S15 established only the first.
> **(3) 22,944-row Hasura ceiling — still UNTESTED**, but **not implicated**: DB total 3,119 is well under it. Remains a read-path question.
> **(4) ✅ DONE 2026-07-30 — reconciled; twin's accounting was the short side.** The 3 Promo-Island-1 rows are twin's own early tests (one impression each; the 8.40/8.53/8.51s were their look-spans). Fullday contributed **zero** — because the circles never participated in the day at all (priority 1's bug), so there was **no fullday Promo shopper** and neither branch of the coordinator's decision tree applies. Not a persistence-loss instance; no oracle discrepancy. *(The separate "oracle said 1, reality 0" is S15 §3.3 — the pre-fix circle standoff, reported at the time.)*
> **(4b) ✅ §1 STOOD DOWN — nobody touched the data.** All 985 Denver sessions closed organically on 07-28 (0/907 close-rule fingerprints; 4 honest 0s stubs from the documented broken-token window). The "~90 orphans" was the coordinator's own unverified S277 claim, **retracted**. Dwell distribution and all downstream oracle comparisons are clean.
> **(5) TWIN-REQ-004 — ✅ ABSORBED core-side 2026-07-30** (`m8trx-shared/status/briefs/BRIEF-CONNECT-READ-SURFACE-2026-07-30.md`), **sequenced behind the Traffic reference surface** → interim positions stay live until it ships, and **`connectSiteScopeAudit` re-runs against every new endpoint on ship** (twin's own named acceptance gate). Filed 2026-07-28 (`m8trx-shared/twin/requirements/TWIN-REQ-004-connect-read-surface.md`): ONE Connect read-surface brief covering task read · space/zone read · impression read (compliance `/state` already shipped as TWIN-REQ-003). Highest-value item is **space/zone identity** — `POST /items/receive` *requires* a `spaceId` nothing on the Connect surface returns, so a third party cannot call receive without out-of-band UUIDs. That gap has a victim who is not twin. **Twin is NOT blocked** (fixture map covers Denver; NATS covers impression verification).
