...
1. **★ M8TRX Connect — live hookup (webhook plane VALIDATED live S9)** — Connect is the canonical path for **both** seeds/updates **and** active interactions. ✅ **Done S9:** API doc absorbed; all 5 P0 simulators built + offline-verified; **inbound webhook plane validated live** against `twin-pos` (`WebhookClient`/`InboundPushDriver` → 200 ack, `connectLiveSmoke`). **Next:** get a **Bearer service key** (`integration:manage` + `scan`/`inventory`) → exercise §6 device driver (scans/receive) + §7 provisioner/health/DLQ (the `twin-pos` X-API-Key is webhook-only; also answers the service-bearer→inventory 401); a **resolving sale** (real `site_id` + EPC/SKU) to transition an item to SOLD; build the SFTP **sshj transport**; then **stand up the parallel ERP/external simulators** injecting `ACTIVITY-PLAN.md` activities. See `reference/connect/LIVE-OPERATIONS.md` for the 24/7 runtime.
