...
| `layout/space-template.json` (160 zones: 11 area + 149 `zone_type='fixture'`) | space + zone provisioning (retail only) | §1 config FKs `space_id`, `zone_id` (fixtures ARE zones — no `fixture_id`) | instantiate 1 space + 160 zones per retail site → config map | ✅ **COVERED (data)** — matches core's zone model; provisioning *format* still core's |
| `stores/*/assortment.csv` (SKUs) | catalog/product master | natural key `sku` → `item.id` | `POST /api/v2/inventory/skus/bulk` (atom #26) + attribs/images | ⚠ **PARTIAL** — bulk SKU exists; **images** are the open `CATALOG-IMPORT-ONBOARDING` gap |
